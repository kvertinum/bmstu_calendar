...
from src.exceptions.keyboard import (
        ...
        for cur_action in actions:
            ...
            for k in cur_action:
                if k not in InlineConstructor.possible_properties:
                    raise UnknownKeyboardButtonPropertyError(
                        unknown_property=k,
                ...
                if len(data) >= InlineConstructor.max_possible_properties:
                    raise TooManyArgsToCreateButtonError(
            ...
            if not all(
                ...
                raise NotEnoughArgsToCreateButtonError(
                    provided_args=list(data.keys()),
            ...
            if "pay" in data:
                if len(btns) != 0 and data["pay"]:
                    raise PaymentButtonMustBeFirstError
                data["pay"] = cur_action["pay"]

            btns.append(InlineKeyboardButton(**data))

        kb = InlineKeyboardMarkup(
            inline_keyboard=create_keyboard_layout(btns, schema),
        )
